x_mesh_back_u = [-1.667, 0, 1.667];
y_mesh_back_u = [2.681, 3.5, 2.681];
z_mesh_back_u = [1.2, 1.2, 1.2];

x_mesh_back_r = [1.429, 1.116, 0.9376, 0.6103];
y_mesh_back_r = [2.298, 1.795, 1.508, 0.9962];
z_mesh_back_r = [0, 0, 0, 0];

x_mesh_back_b = [0.4278, 0.2697, 0.1306, 0, -0.1306, -0.2697, -0.4278, -0.6103];
y_mesh_back_b = [1, 1, 1, 0, 1, 1, 1, 0.9962];
z_mesh_back_b = [0, 0, 0, 0, 0, 0, 0, 0];

x_mesh_back_l = [-0.9376, -1.116, -1.429];
y_mesh_back_l = [1.508, 1.795, 2.298];
z_mesh_back_l = [0, 0, 0];
...
x_mid = [-1.111, -0.7602, -0.386, 0, 0.386, 0.7602, 1.111];
y_mid = [2.598, 2.819, 2.954, 3, 2.954, 2.819, 2.598];
z_mid = [0, 0, 0, 0, 0, 0, 0];
...
v_front = evalin('base', 'x_clicked_front');
y_clicked_front = evalin('base', 'y_clicked_front');
x = v_front(1:25)';
y = y_clicked_front(1:25)';
...
for i = 1:3077
    count = count + 1;
    image_zero (1, 1) = 1;
    if ((ismember(v_front(1, i), x_mesh_back(:)) == 1) && (ismember(v_front(2, i), y_mesh_back(:)) == 1) && (ismember(v_front(3, i), z_mesh_back(:)) == 1))
        ind_x = find(x_mesh_back == v_front(1, i));
        ind_y = find(y_mesh_back == v_front(2, i));
        ind_z = find(z_mesh_back == v_front(3, i));
    ...
    image_zero (2, 1) = v_front(1, i);
    image_zero (3, 1) = v_front(2, i);
    image_zero (4, 1) = v_front(3, i);
    for l = 1:points_clicked
        image_lower (l) = (x_mesh_back(l) - v_front(1, i)) ^ 2 + (y_mesh_back(l) - v_front(2, i)) ^ 2 + (z_mesh_back(l) - v_front(3, i)) ^ 2;
...
x_bar_new_norm_front = abs(x_bar_back)./1280;
y_bar_new_norm_front = abs(1 - (y_bar_back)./720);
